/**
 * pages/Signup.tsx
 * Obsidian Cyber-Terminal Registration
 * Operator registration checkpoint for the Chinook SQL Data Cockpit.
 */
...
export default function Signup() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (password !== confirmPassword) {
      setError("Security error: Password verification keys do not match.")
      return
    }
    ...
    try {
      ...
      setError(err.message || 'Operator registration failed. Please check parameters.')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-surface-950 text-slate-100 flex flex-col lg:flex-row relative selection:bg-brand-500 selection:text-surface-950 font-sans">
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        ...
        <div className="absolute bottom-[20%] right-[10%] w-[400px] h-[400px] bg-brand-600/5 rounded-full blur-[140px]" />
      </div>

      {/* ── Left Column: System Architecture Showcase ────────────────────────── */}
      <div className="lg:w-1/2 p-8 lg:p-16 flex flex-col justify-between border-b lg:border-b-0 lg:border-r border-slate-800 relative z-10 font-mono">
        <div>
          ...
          <div className="max-w-md">
            <div className="inline-flex items-center gap-2 px-2.5 py-1 bg-surface-900 border border-slate-800 text-brand-400 text-[11px] mb-6 uppercase tracking-wider">
              <span>⚡</span>
              <span>OPERATOR_CLEARANCE: REGISTRATION</span>
            </div>

            <h1 className="text-3xl lg:text-5xl font-bold text-white tracking-tight mb-6 font-sans">
              Initialize Operator Clearance.
            </h1>

            <p className="text-slate-400 text-xs leading-relaxed mb-10 font-sans">
              Register a new security clearance to execute natural language database transformations. Unlock dynamic schema pruning, AST syntax compilation, and high-speed TanStack analytics.
            </p>

            {/* Architecture modules checklist */}
            <div className="space-y-3 bg-surface-900/60 p-5 rounded border border-slate-800 text-xs mb-8">
              <div className="text-slate-500 text-[10px] uppercase tracking-wider mb-2">[SYSTEM_CAPABILITIES_INCLUDED]</div>
              <div className="flex items-center justify-between text-slate-300">
                <span>[MOD_01] Token-Optimized RAG Catalog</span>
                <span className="text-brand-400 font-bold">85% SAVINGS</span>
              </div>
              <div className="flex items-center justify-between text-slate-300">
                <span>[MOD_02] Self-Correcting AST Loop</span>
                <span className="text-brand-400 font-bold">0 ERROR GUARD</span>
              </div>
              <div className="flex items-center justify-between text-slate-300">
                <span>[MOD_03] Process-Isolated Python Sandbox</span>
                <span className="text-brand-400 font-bold">ISOLATED EXEC</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="lg:w-1/2 flex items-center justify-center p-6 lg:p-16 relative z-10">
        ...
        >
          ...
          <div className="mb-8 font-mono">
            <span className="text-[10px] text-brand-500 font-bold uppercase tracking-widest block mb-1">// NEW_OPERATOR_REGISTRATION</span>
            <h2 className="text-2xl font-bold text-white tracking-tight font-sans">Create Workspace Profile</h2>
            <p className="text-slate-400 text-xs mt-1 font-sans">Establish your operator clearance parameters.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 font-mono text-xs">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="btn-primary w-full py-3.5 text-xs font-bold uppercase tracking-widest mt-4 flex items-center justify-center gap-2 shadow-lg shadow-brand-500/20"
            >
              {isLoading ? (
                <>
                  <span className="w-4 h-4 border-2 border-surface-950 border-t-transparent rounded-full animate-spin" />
                  <span>[REGISTERING_CLEARANCE...]</span>
                </>
              ) : (
                <>
                  <span>[ESTABLISH_CLEARANCE_PROFILE]</span>
                  <span>→</span>
                </>
              )}
            </button>
          ...
          <div className="mt-8 pt-6 border-t border-slate-800/80 text-center font-mono text-xs text-slate-400">
            <span>Already have Operator Clearance? </span>
            <Link to="/login" className="text-brand-400 hover:text-brand-300 font-bold underline decoration-slate-800 underline-offset-4">
