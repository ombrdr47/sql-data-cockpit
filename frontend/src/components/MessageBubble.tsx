 ...
 * Individual chat message with Framer Motion entry animation.
 * 
 * Upgraded to show:
 *   - AgentProgress (reasoning timeline) — shown during and after streaming
 *   - DataTable (interactive TanStack grid) — shown when SQL results arrive
 *   - SqlPanel (collapsible SQL code)
 *   - ChartView (interactive Plotly chart)
 *   - Retry indicator
 */
    ...
    >
      ...
      <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold
        ${isUser
          ...
          : 'bg-surface-700 border border-slate-600 text-slate-300'
        }`}
      >
        {isUser ? (
          'U'
          ...
          <>
            ...
            {message.approvalRequest && (
              <ApprovalCard
                ...
                onReview={(approved, feedback) =>
                  onReview?.(message.id, approved, feedback)
                }
              />
            )}

            {/* ── SQL Panel — shown only when streaming is done ────────── */}
            {!message.isStreaming && message.generatedSql && (
              <SqlPanel sql={message.generatedSql} />
            )}

            {/* ── Data Table — shown when SQL results are available ─────── */}
